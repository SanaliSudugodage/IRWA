...
#Collects a de-duplicated list of citation IDs across the side’s points to measure diversity of evidence.
...
#Picks a short example snippet and, if available, its citation. Used to make explanations concrete.
...
#Naively splits on periods to estimate sentence count/length for the clarity metric. Lightweight and fast
...
#Creates crude “keywords” when spaCy/NER isn’t available (e.g., in FAST mode), enabling a basic relevance check.
...
#Runs fallacy detectors and penalizes the presence/diversity of fallacy types. Fewer distinct fallacies → higher score; explanation names types and shows an example
...
#Scores based on unique citations and how many points are supported. More diverse citations across more points yield higher scores; the explanation reports counts.
...
#Estimates average sentence length (shorter is clearer) and maps ranges to a 1–5 score. Explanation includes the approximate average and a sample span
...
#Rewards hedging/acknowledgment of limits and penalizes ad hominem language. Produces a balanced 0–5 score with a concrete example
...
#Runs all five scoring functions and returns a list of CriterionScore objects (name, score, explanation) for one side
...
#Deterministic tie-breaker: first prefers more unique citations, then more supported points; if still tied, picks “pro” to be deterministic.
