...
#Exposes a FastAPI POST /agents/counter endpoint.
"""For each opponent point, it:

Scores the weakness (simple heuristics: missing citations, fallacies). fallacy.py

Generates a counterclaim (IR-grounded if possible, else generic).

Proposes an improved version of the argument (again IR-grounded if possible)."""
...
#Utility to trim long strings to n chars with an ellipsis
...
No citations → +0.5 weakness

Only 1 citation → +0.2 weakness

Fallacies detected → +0.5 weakness

Caps at 1.0.

Returns:

A numeric weakness score,

The list of fallacy types (if any),
...
#Extracts entities/keywords from the opponent’s claim and combines with the motion. Produces a compact query string (semicolon-separated) to drive IR search.
...
#Prompts the LLM for a short, neutral counter-argument focusing on caveats and assumptions; if LLM fails, returns a safe template rebuttal.
...
#Asks the LLM to rewrite a narrower, more testable version of the stance with one explicit limitation. Falls back to a generic “narrow and state assumptions” sentence.
...
#Uses top IR snippets to ground a 2–3 sentence counter via the LLM (no hallucinated specifics). If the LLM fails, returns a cautious evidence-referencing template
...
#Accepts either structured opponent or legacy opponent_points, sanitizes all text/citations, and returns a uniform Opponent object for the rest of the pipeline.
