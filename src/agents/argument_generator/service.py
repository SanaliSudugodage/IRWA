...
#create router which required api key and build single llm instance
...
#If index or module is missing, logs a warning and returns None so the rest of the code can fall back to no-IR cleanly.
...
#Extracts entities/keywords from the motion using NLP
...
#Get retriever; if unavailable → fall back to _make_points_no_ir.
#Build query terms from motion/stance.
#For each term, call retriever.search(q, k=4); collect unique top hits
#If we got no hits → fallback to _make_points_no_ir.
#Sort hits by score, take the top max_points
"""For each hit:

If LLM is enabled → ask it to write one concise claim only from the passage (guardrails: 25–45 words, neutral, no speculation).

If LLM fails/unavailable → use summarize(p.text) as a fallback. summarizer.py

Create an ArgumentPoint(text=claim, citations=[f"{doc_id}#{chunk_id}"])."""
...
#LLM-only path when IR is OFF or thin
#If LLM isn’t available → build generic statements by summarizing the motion and sprinkling a couple of extracted keywords
